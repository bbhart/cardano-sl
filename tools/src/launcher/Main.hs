...
instance FromJSON LauncherOptions where
    parseJSON = withObject "LauncherOptions" $ \o ->
        genericParseJSON defaultOptions $ Object $
            o <> HM.singleton "nodeArgs" (Array mempty)
              <> HM.singleton "walletArgs" (Array mempty)
              <> HM.singleton "updaterArgs" (Array mempty)
...
spawnNode (path, args, mbLogPath) = do
    ...
    logNotice $ sformat ("    Binary path: "%string) path
    logNotice $ sformat ("    Arguments: "%stext) $ unwords $ map quote args
